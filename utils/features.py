...
def get_handcraft_features(P):
        ...
        if (len(P[0].shape) == 2):
            ...
            dists = get_finger_palm_distance(P.cpu())
        # dists = [d[:,:3] for d in dists]
        dists = [dist.mean(axis=1) for dist in dists]
        peak_idxs, peak_vals = get_cycle_peaks(dists, keep=10, savgol_win=5, prominence=0.10, min_peak_dist=10)
        valley_idxs, valley_vals = get_cycle_valleys(dists, peak_idxs, savgol_win=5)
        peak_idxs, peak_vals, valley_idxs, valley_vals, peak_width, valley_width = adjust_peaks(dists, peak_idxs, peak_vals, valley_idxs, valley_vals)
        ...
        # UPDRS features
        num_hesitations = get_UPDRS_num_hesitations(peak_idxs, peak_vals, 
                                                                min_num_peaks, hesitation_resid_thresh, score=False)
        amp_dec_idxs = get_UPDRS_amplitude_decrement(peak_vals, min_num_peaks, 
                                                                    amp_dec_thresh, score=False)
        slowings = get_UPDRS_slowing(peak_idxs, min_num_peaks, score=False)
        
        # Catch 22 features
        catch24_feats = get_catch22_features(dists)
        
        # Cycle features
        cycle_feats = get_cycle_features(dists, peak_idxs, peak_vals, valley_vals)
        ...
        for i in range(len(dists)):
            ...
            if len(cycle_feats[0][i]) > 0:
                ...
                all_features[i].append(get_hesitations_peaks_th(peak_vals[i], threshold=1))
                all_features[i].append(get_hesitations_valleys_th(valley_vals[i], threshold=0.5))
